...
#define TAG "com.geraudbourdin.infrared"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> TiInfrared_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TiInfrared.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TiInfraredBindings::lookupGeneratedInit(
...
static void TiInfrared_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TiInfrared_getBinding)->GetFunction());
}

static void TiInfrared_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiInfraredBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiInfraredBinding = {
	"com.geraudbourdin.infrared",
	TiInfrared_init,
	TiInfrared_dispose
};
...
Java_com_geraudbourdin_infrared_TiInfraredBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.geraudbourdin.infrared", &TiInfraredBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiInfraredBindings::lookupGeneratedInit));
}
